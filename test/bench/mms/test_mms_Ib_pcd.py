...
@pytest.mark.parametrize("pcd_variant", ["BRM2",]) # "BRM1",
@pytest.mark.parametrize("ls", ["direct",])
def test_scaling_time(pcd_variant, ls, matching_p, postprocessor):
    """
    ...
    for m in range(5): # CHANGE #1: set "m in range(7)"
        dt = 0.1*0.5**m
        label = "{}_{}_dt_{}_{}".format(pcd_variant, ls, dt, basename)
    ...
    postprocessor.pop_items(["ndofs", "tmr_prepare", "scheme"])
...
class Postprocessor(GenericBenchPostprocessor):
    def __init__(self, t_end, level, OTD, OPA, outdir, averaging=True):
    ...
    def flush_plots(self):
        ...
        for fixed_vars, fig in six.iteritems(self.plots):
            ...
            for free_vars, datapoints in six.iteritems(data):
                ...
                ys2 = datapoints["ys2"]
                ys3 = datapoints["ys3"]
                self._plot(fig, xs, ys0, ys1, ys2, ys3, free_vars, self.OTD, style)
            self._save_plot(fig, fixed_vars, self.outdir)
        self.results = []

    @staticmethod
    def _plot(fig, xs, ys0, ys1, ys2, ys3, free_vars, OTD, style):
