...
def test_bubble(ls, pcd_variant, matching_p, case, postprocessor):
    ...
    for level in range(1, 2): # CHANGE #1: set " level in range(4)"
        ...
        label = "case_{}_{}_{}_level_{}_dt_{}_{}".format(
                    case, pcd_variant, ls, level, dt, basename)
        with Timer("Prepare") as tmr_prepare:
            ...
            model.parameters["rho"]["trunc"] = "minmax" #"clamp_soft"
            model.parameters["nu"]["itype"] = "har"
            model.parameters["nu"]["trunc"] = "minmax" #"clamp_hard"
        ...
        with Timer("Time stepping") as tmr_tstepping:
            try:
                result = TS.run(t_beg, t_end, dt, OTD, it=0)
            except:
        ...
        print(label, result["ndofs"], result["h_min"],
              result["it"], result["tmr_tstepping"], result["krylov_it"])
