...
@pytest.mark.parametrize("mo", [4.0e-3, 4.0e-5, 4.0e-7, 4.0e-9])
def test_bubble(case, mo, method, scheme, matching_p, postprocessor):
    ...
    for level in [2,]: # CHANGE #1: set " level in [3,]"
        ...
        label = "case_{}_mo_{}_{}_{}_level_{}_k_{}_dt_{}_{}".format(
                    case, mo, scheme, method, level, k, dt, basename)
