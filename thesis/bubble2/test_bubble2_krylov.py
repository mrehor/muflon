...
@pytest.mark.parametrize("THETA2", [1.0,])
def test_bubble(case, THETA2, method, scheme, matching_p, postprocessor):
    ...
    for level in range(4): # CHANGE #1: set " level in [3,]"
        ...
        with Timer("Prepare") as tmr_prepare:
            ...
            if scheme == "FullyDecoupled" or method == "lu":
                ...
                #model.parameters["full"]["factor_nu0"] = 5.A
            ...
            # Prepare solver
            comm = mesh.mpi_comm()
            #set_log_level(INFO) if MPI.rank(comm) == 0 else set_log_level(INFO+1)
